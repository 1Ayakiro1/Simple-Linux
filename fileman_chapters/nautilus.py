...
scrolled_window_nautilus = Gtk.ScrolledWindow()
scrolled_window_nautilus.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)

# Main Box in scrolled window
main_box_panel_nautilus = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
main_box_panel_nautilus.set_valign(Gtk.Align.START)
main_box_panel_nautilus.add_css_class("intro-list")
...
main_box_panel_nautilus.append(back_button)

# Label
label = Gtk.Label(label=fileman_translations[current_language]["nautilus"])
label.set_use_markup(True)
dynamic_labels.append((label, "nautilus"))
label.add_css_class("nautilus")
label.set_wrap(True)
label.set_xalign(0)
main_box_panel_nautilus.append(label)

scrolled_window_nautilus.set_child(main_box_panel_nautilus)

def get_nautilus_panel():
    return scrolled_window_nautilus, "nautilus_panel", back_button

def get_nautilus_text():
    return fileman_translations[current_language]["nautilus"]