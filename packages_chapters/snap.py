...
scrolled_window_snap_topic = Gtk.ScrolledWindow()
scrolled_window_snap_topic.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)

# Main Box in scrolled window
main_box_panel_snap_topic = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
main_box_panel_snap_topic.set_valign(Gtk.Align.START)
main_box_panel_snap_topic.add_css_class("intro-list")
...
main_box_panel_snap_topic.append(back_button)

# Label
label = Gtk.Label(label=packages_translations[current_language]["snap"])
label.set_use_markup(True)
dynamic_labels.append((label, "snap"))
label.add_css_class("snap")
label.set_wrap(True)
label.set_xalign(0)
main_box_panel_snap_topic.append(label)

scrolled_window_snap_topic.set_child(main_box_panel_snap_topic)

def get_snap_panel():
    return scrolled_window_snap_topic, "hotkeys_intro_panel", back_button